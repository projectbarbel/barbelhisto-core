...
public class JournalUpdateStrategyKeepSubsequentTest {
    ...
    public void testApply_KeepSubsequent() throws Exception {
        VersionUpdate update = BarbelHistoFactory.createDefaultVersionUpdate(BarbelHistoBuilder.barbel().withMode(BarbelMode.BITEMPORAL), (Bitemporal)journal.list().get(3)).prepare().effectiveFrom(clock.now().toLocalDate()).get();
        ...
        JournalUpdateStrategyKeepSubsequent updateFunction = new JournalUpdateStrategyKeepSubsequent("system");
        List<Object> updateVersion = updateFunction.apply(journal, update.result());
        assertTrue(((DefaultDocument)result.newSubsequentVersion()).getData().equals("some new data"));
        assertTrue(((DefaultDocument)result.newPrecedingVersion()).getData().equals(((DefaultDocument)journal.list().get(3)).getData()));
    ...
    public void testApply_KeepSubsequent_subsequentPeriodsExist_andShouldNotBeInactivated() throws Exception {
        VersionUpdate update = BarbelHistoFactory.createDefaultVersionUpdate(BarbelHistoBuilder.barbel().withMode(BarbelMode.BITEMPORAL), (Bitemporal)journal.list().get(2)).prepare().effectiveFrom(LocalDate.of(2018, 7, 1)).get();
        assertTrue(journal.read().activeVersions().size()==4);
        VersionUpdateResult result = update.execute();
        ((DefaultDocument)result.newSubsequentVersion()).setData("some new data");
        JournalUpdateStrategyKeepSubsequent updateFunction = new JournalUpdateStrategyKeepSubsequent("system");
        List<Object> updateVersion = updateFunction.apply(journal, update.result());
        assertTrue(((DefaultDocument)result.newSubsequentVersion()).getData().equals("some new data"));
        assertTrue(((DefaultDocument)result.newPrecedingVersion()).getData().equals(((DefaultDocument)journal.list().get(2)).getData()));
        assertTrue(updateVersion.size()==2);
        assertTrue(journal.read().inactiveVersions().size()==1);
    }
    ...
    public void testApply_KeepSubsequent_multiplieSubsequentPeriodsExist_andShouldNotBeInactivated() throws Exception {
        VersionUpdate update = BarbelHistoFactory.createDefaultVersionUpdate(BarbelHistoBuilder.barbel().withMode(BarbelMode.BITEMPORAL), (Bitemporal)journal.list().get(0)).prepare().effectiveFrom(LocalDate.of(2016, 7, 1)).get();
        assertTrue(journal.read().activeVersions().size()==4);
        VersionUpdateResult result = update.execute();
        ((DefaultDocument)result.newSubsequentVersion()).setData("some new data");
        JournalUpdateStrategyKeepSubsequent updateFunction = new JournalUpdateStrategyKeepSubsequent("system");
        List<Object> updateVersion = updateFunction.apply(journal, update.result());
        assertTrue(((DefaultDocument)result.newSubsequentVersion()).getData().equals("some new data"));
        assertTrue(((DefaultDocument)result.newPrecedingVersion()).getData().equals(((DefaultDocument)journal.list().get(0)).getData()));
        assertTrue(updateVersion.size()==2);
        assertTrue(journal.read().inactiveVersions().size()==1);
    }
