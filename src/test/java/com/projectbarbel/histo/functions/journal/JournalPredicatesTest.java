...
public class JournalPredicatesTest {
    ...
    public void testEffectiveAfter_isAfter_dueDateBeforeEffectivePeriod() throws Exception {
        DefaultDocument doc = DefaultDocument.builder().withData("bla").withVersionId("id").withBitemporalStamp(BitemporalStamp.of("test", "docid", EffectivePeriod.create().from(LocalDate.now().plusDays(1)).toInfinite(), RecordPeriod.create("me"))).build();
    ...
    public void testEffectiveAfter_isAfter_dueDateOnEffectiveFrom() throws Exception {
        DefaultDocument doc = DefaultDocument.builder().withData("bla").withVersionId("id").withBitemporalStamp(BitemporalStamp.of("test", "docid", EffectivePeriod.create().from(LocalDate.now()).toInfinite(), RecordPeriod.create("me"))).build();
        assertTrue(new JournalPredicates(LocalDate.now()).effectiveAfter(doc));
    }

    @Test
    public void testEffectiveAfter_notAfter_dueDateInEffectivePeriod() throws Exception {
        DefaultDocument doc = DefaultDocument.builder().withData("bla").withVersionId("id").withBitemporalStamp(BitemporalStamp.of("test", "docid", EffectivePeriod.create().from(LocalDate.now().minusDays(1)).toInfinite(), RecordPeriod.create("me"))).build();
    ...
    public void testEffectiveAfter_notAfter_dueDateAfterEffectivePeriod() throws Exception {
        DefaultDocument doc = DefaultDocument.builder().withData("bla").withVersionId("id").withBitemporalStamp(BitemporalStamp.of("test", "docid", EffectivePeriod.create().from(LocalDate.now().minusDays(10)).until(LocalDate.now().minusDays(1)), RecordPeriod.create("me"))).build();
        assertFalse(new JournalPredicates(LocalDate.now()).effectiveAfter(doc));
    }

    @Test
    public void testEffectiveAfter_notAfter_dueDateOnEffectiveUntil() throws Exception {
        DefaultDocument doc = DefaultDocument.builder().withData("bla").withVersionId("id").withBitemporalStamp(BitemporalStamp.of("test", "docid", EffectivePeriod.create().from(LocalDate.now().minusDays(10)).until(LocalDate.now()), RecordPeriod.create("me"))).build();
        assertFalse(new JournalPredicates(LocalDate.now()).effectiveAfter(doc));
    }
